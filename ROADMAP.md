...
### M9 backlog: interactive moves editor + auto-scramble + decoupled solve flow
Recast the app's interaction model so editing is the primary surface and Solve becomes just one of several ways to populate moves. Concrete changes: (a) **Auto-scramble on page load** — default state is a freshly-generated scramble, not solved. Refresh always gives the user something to work with; the existing "scramble" button becomes a reset/re-scramble. (b) **Auto-solve after scramble** (or behind a toggle / setting) — paste-or-load-then-watch is the common path; the user is tired of clicking Solve every time. (c) **Decouple solve → cards via an editable moves field**. Today the solve action populates the cards directly. Recast: solve populates a `moves` text field (e.g. `"R U R' U' F R F'"`); cards are *derived* from that field. User can clear / delete / hand-type moves and cards update live as they edit (each keystroke that resolves to valid notation appends/removes a card). Solve re-populates the moves text and cards re-render. This makes the bottom-of-page section into a dynamic, interactive playground rather than a static result view. (d) **Make it fun** — the meta-goal: the app should feel like an REPL/sandbox, not a one-shot solver demo. Animation system (M9.2) is a prerequisite — once cards animate their move, hand-typing R then watching the card animate is the payoff loop. Surfaced 2026-05-11 during Block C eyeball. Probably its own milestone (M9.4?) or a substantial Block E inside M9.2 after the animation system stabilizes.
Surfaced: 2026-05-11
