...
### M9 1B': animation visual model exploration (successor to paused 1B)
Block 1B paused 2026-05-10 after 4 P0 preview revisions failed to converge on a 2D animation visual model the user accepts. The iterate-fast cadence within one block proved too tight for the design space. This successor block opens fresh, dedicated solely to nailing the visual model — **preview-only iteration** on `web/preview/flat-cube-animated.html`, no production wiring. Substrate is rev4 (`3b6998c` on branch `m9-per-card-animations`, unmerged): 12 panels all from solved state, cross-net mask clipping, conveyor-along-edge-axis flow with empirical source-projection direction rule. **Open question (the gate):** "where does the new strip come from?" The empirical source-projection rule maps cubie source faces onto 2D net positions and picks flow direction by projection onto the active edge axis. For R|B this gives DOWN (new arrives from above B). User describes a different mental model — cubies "theoretically wrap around the outside of the cube" with 3D-rotation subtleties that may not reduce to a single per-(turnedFace, adjacentFace) direction; hints: "each row/col of a face could have different directions." Possible richer models to explore: per-sticker direction within an active edge (not just per-edge); arc-projection that traces the 3D rotation through the unfolded net rather than position-projection; explicit hand-coded direction tables per (turnedFace, adjacentFace, sticker) triple. **What's kept from 1B:** conveyor-along-edge-axis is the right general shape, cross-net mask is the right clipping approach, 12-panel all-from-solved is the right inspection set, 400 ms pre-pause + 600 ms ease + click-replay is the right interaction loop. **What's open:** the flow direction model; whether per-(face, sticker) or just per-(face, edge); how to depict 3D rotation arcs in 2D for non-net-adjacent face transfers. **Once the visual model is locked,** the existing `plans/m9-per-card-animations.md` resumes at P1 (FlatCubeRenderer learns animation) → P2 → P3 → P4 in a subsequent block. Branch `m9-per-card-animations` stays alive as reference / starting substrate.
Surfaced: 2026-05-10
