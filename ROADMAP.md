...
- **M11** — ONNX export + browser inference — *active* ([plan](plans/m11-onnx-browser.md)). Take the current 3x3 champion checkpoint, export to ONNX, port beam search to TypeScript, run end-to-end in the browser via `onnxruntime-web`. Both paths (FastAPI/MPS and ONNX/browser) stay live behind a shared `Solver` abstraction with a UI toggle. Capstone goal: a fully-static Vercel-deployable demo + earned intuition about browser-side ML inference perf vs the server baseline.
  - Block A — ONNX export + Python-side parity gate — *active*
