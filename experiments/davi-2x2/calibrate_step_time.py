...
Output goes to
``experiments/davi-2x2/runs/<UTC-ts>_calibration/calibration.jsonl`` plus
a one-line stdout digest per cell.

Without these numbers every downstream tier (1: budget per sanity run;
2: budget per swept cell; 3: champion run wall-time projection) is
blind. Run this once, refine the grid if needed, then run it again as
a baseline check whenever ``ValueNet`` or ``davi_step`` changes shape.
"""
...
# Calibration grid — spans tier 1 (downscaled) through tier 3 (full).
# Edit here to widen / narrow; 18 cells = ~5–15 min on M4 Max at trials=20.
GRID_BATCH_SIZES: tuple[int, ...] = (256, 1000, 4000)
GRID_BODY_WIDTHS: tuple[tuple[int, int], ...] = (
    (256, 64),
    (1000, 256),
    (5000, 1000),
)
GRID_N_RESIDUAL_BLOCKS: tuple[int, ...] = (2, 4)

# DAVI is *not* sensitive to scramble depth at the per-step-time level —
# `apply_all_moves` is fixed-cost; depth only changes which scrambled state
# you start from. Hold it at the M5 acceptance value.
SCRAMBLE_DEPTH: int = 14
